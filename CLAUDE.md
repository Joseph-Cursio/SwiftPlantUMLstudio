...
# Run all tests
xcodebuild test -scheme SwiftPlantUMLstudio -destination 'platform=macOS,arch=arm64'

# Run a single unit test
xcodebuild test -scheme SwiftPlantUMLstudio -destination 'platform=macOS,arch=arm64' \
  -only-testing:SwiftPlantUMLstudioTests/SwiftPlantUMLstudioTests/<TestName>

# Run a single UI test
xcodebuild test -scheme SwiftPlantUMLstudio -destination 'platform=macOS,arch=arm64' \
  -only-testing:SwiftPlantUMLstudioUITests/SwiftPlantUMLstudioUITests/<TestName>
```

- **Target platform**: macOS 26.4+, `SDKROOT = macosx`
- **Bundle ID**: `name.JosephCursio.SwiftPlantUMLstudio`
...
The **CLI** (`swiftumlbridge`) is a thin wrapper over the framework. The **macOS Studio app** (`SwiftPlantUMLstudio`) is a SwiftUI GUI front-end over the same framework.
...
- **UI tests** use XCTest (`SwiftPlantUMLstudioUITests`).
